...
**安全策略**：单个计划一次任务中每个候选座位最多尝试一次（最多 6 个候选座位，旧计划的更大值会被自动钳制），两次提交之间有短暂随机间隔并加抖动；只有超星明确返回“座位不可用”才会换座重试。提交后超时、响应异常或结果不明时会停止并要求人工核实（能核实超星端时自动核实），绝不自动重复提交；滑块和点选验证码均只报告并安全停止，不自动求解。
