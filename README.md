...
## 本地网页版说明（app/ 目录，当前主要使用方式）

本仓库在原 CLI 脚本之上改造出本地网页版：FastAPI + SQLite，仅监听 localhost，支持多账号（DPAPI 加密存储）、候选座位优先级、定时计划、只读探测和运行记录。

**预约参数来源（重要）**：程序登录后会自动发现目标日期选座页所需参数并在每次执行时取最新 token；不需要日常手填链接。若学校未暴露可自动发现的上下文，仍兼容旧版当前状态页取参；当前页显示“有人使用”不会再直接判定未来日期不可预约。选座链接保留为高级兜底覆盖。

**统一自动预约（v11）**：每个账号独立提供“检查登录／提前登录”，有效会话复用于定时预约；前 10 分钟检查登录，前 30 秒准备页面，到点自动提交。无需验证码时自动完成，需要操作时恢复原窗口。安装与使用见 [LOCAL_WEB.md](LOCAL_WEB.md)。

本次升级的实现范围、测试证据和正式计划剩余验收见 [UPGRADE_STATUS.md](UPGRADE_STATUS.md)。管理页统一使用自动预约流程，不再选择接口或浏览器模式。

**2026-09-11 漏跑修复**：临近开抢补跑、历史中断误标、浏览器请求闸门及回归验证见 [BUGFIX_20260911.md](BUGFIX_20260911.md)。生产预约走官方浏览器页面，验证码由用户完成、明确风控立即停止。下文旧 CLI、滑块与 API 竞速说明不代表当前网页版能力；`fire_accept_calibration`、并发 volley、racer 自愈仅属于保留的旧 API 路径，未重新启用。`upgrade_check.py` 验证旧 API／共享逻辑，不能代替浏览器验收或真实抢座结果。


**探测**：计划卡片的“自动发现参数”与“探测”均只读检查（不提交预约）：返回目标日期参数状态和来源；当前座位被占用不会再误报为目标日期已不可预约。
